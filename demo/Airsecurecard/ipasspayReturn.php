...
use Ipasspay\AirsecurecardChannel\service\AirsecurecardService;
    ...
    //但不建议在此页面做实际的交易业务数据处理，因为该页面是airsecurecard直接提供给支付用户浏览器做跳转的，用户不一定会被访问，且数据安全性和时效性都不能保证。
    echo "airsecurecard同步结果返回\n";
    ...
    echo '验签字符串为'.$airsecurecard_service->getVerifySignString()."\n";
    echo '验签为'.$airsecurecard_service->getVerifySign()."\n";

    //可以直接使用php获得请求参数的方法，获得参数进行处理......
    echo '同步通知数据：'.json_encode($_REQUEST)."\n";
    //也可使用sdk中的方法，获得参数进行处理......
    echo 'sdk中数据为'.json_encode($airsecurecard_service->getNotifyData())."\n";
    //--------------------
    exit;
